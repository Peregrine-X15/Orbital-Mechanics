...
e_F=(norm(r2)-norm(r1))/chord;

a_F=0.5*(r_Earth_o_mag+r_Mars_f_mag);
p_F=a_F*(1-e_F^2);

true_anom=linspace(0, 2*pi, 500);

r_FE=p_F./(1+e_F*cos(true_anom));

x_FE=r_FE.*cos(true_anom);
y_FE=r_FE.*sin(true_anom);
z_Fe=zeros(size(x_FE));

%earth and mars
earth_orbit_x = r_Earth_o_mag * cos(true_anom);
earth_orbit_y = r_Earth_o_mag * sin(true_anom);
earth_orbit_z = zeros(size(earth_orbit_x)); % 2D plot for Earth

% Plot Mars orbit
mars_orbit_x = r_Mars_f_mag * cos(true_anom) ;
mars_orbit_y = r_Mars_f_mag * sin(true_anom);
...
plot3(mars_orbit_x,mars_orbit_y,mars_orbit_z,'r-','DisplayName','Mars Orbit')
plot3(earth_orbit_x,earth_orbit_y,earth_orbit_z,'b-','DisplayName','Earth Orbit')
plot3(r2(1), r2(2),r2(3), 'ro', 'MarkerSize', 10, 'DisplayName', 'Mars Position');
plot3(x_FE, y_FE,z_Fe, 'm-', 'LineWidth', 1.5, 'DisplayName', 'Fundamental Ellipse');

% Plot settings
xlabel('X Position (km)');
ylabel('Y Position (km)');
title('Fundamental Ellipse of Earth to Mars');
legend('show');
grid on;
axis equal;
hold off;